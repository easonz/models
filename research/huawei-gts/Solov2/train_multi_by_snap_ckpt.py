...
def main():
    ...
    # 获取最新的ckpt的snapshot文件
    ckpt_file = f'{cfg.work_dir}/latest.ckpt'
    file_name = osp.basename(osp.realpath(ckpt_file))
    print(f'===[ReloadCKPT]==== Read snapshot ckpt, file name: {file_name}')

    ms.load_checkpoint(ckpt_file, model)

    optimizer = make_optimizer(model, cfg.optimizer)

    # 获取最后一次执行的epoch与iter数
    epoch = int(file_name.split('_')[1])
    iter = int(file_name.split('_')[3])
    print(f'===[ReloadCKPT]==== Read snapshot ckpt with epoch: {epoch}, iter: {iter}')
    runner = Runner(model, optimizer, batch_processor, work_dir=cfg.work_dir, epoch=epoch, iter=iter)
