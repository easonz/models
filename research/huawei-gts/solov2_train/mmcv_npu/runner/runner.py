...
class Runner(nn.TrainOneStepWithLossScaleCell):
    ...
    def train(self, dataset):
        ...
        for batch,data in enumerate(dataset):
            ...
            #import pdb;pdb.set_trace()
            #logging.info(f"data:{data}, sie : {len(data)}")
            #print(batch)
            import time
            begintime = time.time()
            '''if data[0]['img_meta'].data[0][0]['filename'] != 'data/coco/train2017/000000391895.jpg':
                continue'''
            # logging.info(f"batch {batch}, img_mata{data[0]['img_meta']}")
            # logging.info(f"{data}")
            ...
            if batch % 1 == 0:
                ...
                # print(f"loss: {loss:>7f} lr: {self.optimizer.lrs[0].value().asnumpy():>7f} [{current:>3d}/{size:>3d}], steptime: {endtime-begintime}")
