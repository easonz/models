...
ms.set_context(device_target="Ascend", mode=ms.GRAPH_MODE, device_id=config.device_id, jit_syntax_level=ms.STRICT)
...
if __name__ == '__main__':
    ...
    # Step2: 初始化模型与优化器, 这里需要定义权重文件路径或者不输入通过网络下载
    ...
    for epoch in range(epochs):
        ...
        with tqdm(total=train_dataset.get_dataset_size()) as t:
            for batch, (token_ids, seq_len, labels) in enumerate(train_dataset.create_tuple_iterator()):
                ...
                print(f'========[PT][loss] loss: {loss}, cost: {(datetime.now() - now)}')
                t.update(1)
